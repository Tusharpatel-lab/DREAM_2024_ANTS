...
df_train <- read_csv("/misc/pichu/data/DREAM_24/Raw_data/Beta_raw_subchallenge2.csv") %>% as.data.frame()
...
detp <- read_csv("/misc/pichu/data/DREAM_24/Raw_data/DetectionP_subchallenge2.csv") %>% as.data.frame()
...
ano <- read.csv("/misc/pichu/data/DREAM_24/Raw_data/Sample_annotation.csv")
...
ano <- ano[ano$Array == "850K", ]

# Ensure df_train and detp columns match the rownames of ano
df_train_sub <- df_train[, rownames(ano)]
detp <- detp[rownames(df_train_sub), rownames(ano)]
...
beta_train_filter <- df_train[rownames(beta_train_filter),]

# saveRDS(beta_train_filter, "/misc/pichu/data/DREAM_24/Processed_data/850k_all_samples/filtered_beta.rds")
...
probeInfoALL.lv <- readRDS("/misc/pichu/data/DREAM_24/Raw_data/probe_info_850k.rds")
...
# Apply the champ.BMIQ function to each column (in parallel) of beta_train_filter and normalize the beta values and change the colnames and rownames
cl <- makeCluster(10)
registerDoParallel(cl)

beta_norm_BMIQ <- as.data.frame(foreach(x = 1:ncol(beta_train_filter), .combine = cbind, .export = c("champ.BMIQ", "blc")) %dopar% 
                                  champ.BMIQ(beta_train_filter[,x], design.v, sampleID = colnames(beta_train_filter)[x])$nbeta)
rownames(beta_norm_BMIQ) <- rownames(beta_train_filter)
colnames(beta_norm_BMIQ) <- colnames(beta_train_filter)

# saveRDS(beta_norm_BMIQ, "/misc/pichu/data/DREAM_24/Processed_data/850k_all_samples/filtered_custom_BMIQ_normalized_beta.rds")

# Calculate Spearman correlations between CpGs and gestational age (GA)
...
beta_norm_BMIQ_corr <- rbind(top_2000, bottom_2000)

aging_cpg_ewas <- read.csv("/misc/pichu/data/DREAM_24/Raw_data/aging-11-102049-s003.csv")

#Subset to get a control matrix for control samples and filter only EWAS CpGs afterwards
beta_norm_BMIQ_ewas <- beta_norm_BMIQ[aging_cpg_ewas$Name, ]
beta_norm_BMIQ_ewas <- beta_norm_BMIQ_ewas[complete.cases(beta_norm_BMIQ_ewas), ]

beta_norm_BMIQ <- rbind(beta_norm_BMIQ_ewas, beta_norm_BMIQ_corr)
beta_norm_BMIQ <- unique(beta_norm_BMIQ)

# Store the row names of the subset beta values
second_filter <- rownames(beta_norm_BMIQ)

# Train a regularized linear regression model using cross-validation
model <- cv.glmnet(x = as.matrix(t(beta_norm_BMIQ)), y = log(ano$GA), nfolds = 30, alpha = 0.1, family = "gaussian")
