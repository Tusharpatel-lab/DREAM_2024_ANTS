...
# Define --input option: specifies the input directory, default is /input
# Define --output option: specifies the output directory, default is /output
option_list <- list(
...
# Create an OptionParser object with the defined options
...
test_data <- as.data.frame(read_csv(file.path(opt$input, "Leaderboard_beta_subchallenge2.csv")))
...
probeInfoALL.lv <- readRDS("/usr/local/bin/probe_info_850k.rds")
...
model <- readRDS("/usr/local/bin/model_test_SC2.rds")
